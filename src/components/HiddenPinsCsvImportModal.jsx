...
import { HIDDEN_CSV_HEADERS } from '../utils/hiddenPinsCsv'
...
/**
 * Hidden pins CSV Import preview / validation modal.
 */
export function HiddenPinsCsvImportModal({ validation, onConfirm, onClose }) {
  ...
  const handleConfirm = () => {
    ...
    if (hasDuplicates && !overwrite) {
      const proceed = window.confirm(
        `${duplicates.length} hidden pin(s) already exist and will be skipped.\n\nImport anyway (keep existing data)?`,
      )
      if (!proceed) {
        return
      }
      ...
      >
        <header className="csv-import-head">
          <div>
            ...
            <h2 id={headingId}>Review hidden pins</h2>
        ...
        <div className="csv-import-body">
          <div className="csv-import-stats">
            <span className="csv-import-stat">
              <strong>{stats.newCount ?? 0}</strong> new hidden pin(s)
            </span>
            <span className="csv-import-stat">
          ...
          {hasDuplicates ? (
            <div className="csv-import-block">
              <h3>Duplicate hidden pins</h3>
              <p className="csv-import-hint">
                These rows match existing hidden entries (Kakao Place ID, coordinates, or name pair).
              </p>
              <fieldset className="csv-import-overwrite-group">
                <label className="csv-import-overwrite">
                  <input
                    type="radio"
                    name="hidden-dup-mode"
                    value="skip"
                ...
                <label className="csv-import-overwrite">
                  ...
                  />
                  Overwrite existing hidden entries
                </label>
              </fieldset>
          ...
          <div className="csv-import-block">
            <h3>Preview</h3>
            <p className="csv-import-hint">
              Expected columns: <code>{HIDDEN_CSV_HEADERS.join(', ')}</code>
            </p>
            <div className="csv-import-table-wrap">
              <table className="csv-import-table">
                <thead>
                ...
                <tbody>
                  {previewSlice.map((row) => (
                    <tr key={row.rowNum} className={row.valid ? '' : 'invalid'}>
                      ...
                      <td>
                        <span
                          className={`csv-import-status${row.status === 'active' ? ' published' : ''}`}
                        >
                          {row.status}
                        </span>
                      </td>
          ...
          <p className="csv-import-note">
            <strong>active</strong> = hidden from all modes · <strong>inactive</strong> = visible to
            users (kept in CSV for ops)
          </p>
        </div>
          ...
          >
            {canImport
              ? hasDuplicates && overwrite
                ? `Import & overwrite ${duplicates.length} hidden pin(s)`
                : `Import ${validRows.length} hidden pin(s)`
...
export default HiddenPinsCsvImportModal
