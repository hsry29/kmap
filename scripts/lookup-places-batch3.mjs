import fs from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')
for (const line of fs.readFileSync(path.join(ROOT, '.env'), 'utf8').split(/\r?\n/)) {
  const m = line.match(/^\s*([A-Z0-9_]+)\s*=\s*(.*?)\s*$/)
  if (m && !process.env[m[1]]) process.env[m[1]] = m[2].replace(/^['"]|['"]$/g, '')
}
const KEY = process.env.KAKAO_REST_API_KEY

const places = [
  { en: 'Yeouido Hangang Park', queries: ['여의도한강공원', 'Yeouido Hangang Park'] },
  { en: 'IFC Mall', queries: ['IFC몰', 'IFC Mall 서울', '서울국제금융센터'] },
  { en: 'The Hyundai Seoul', queries: ['더현대서울', 'The Hyundai Seoul'] },
  { en: 'Nodeul Island', queries: ['노들섬', 'Nodeul Island'] },
  { en: 'Banpo Hangang Park', queries: ['반포한강공원', 'Banpo Hangang Park'] },
]

const SEOUL = { lat: 37.5665, lng: 126.978 }
const sleep = (ms) => new Promise((r) => setTimeout(r, ms))

async function search(query) {
  const url = new URL('https://dapi.kakao.com/v2/local/search/keyword.json')
  url.searchParams.set('query', query)
  url.searchParams.set('x', String(SEOUL.lng))
  url.searchParams.set('y', String(SEOUL.lat))
  url.searchParams.set('radius', '20000')
  url.searchParams.set('size', '5')
  const res = await fetch(url, { headers: { Authorization: `KakaoAK ${KEY}` } })
  return (await res.json()).documents || []
}

for (const place of places) {
  let docs = []
  for (const q of place.queries) {
    docs = await search(q)
    if (docs.length) break
    await sleep(100)
  }
  const best = docs[0]
  if (!best) {
    console.log(`${place.en}: NOT FOUND`)
    continue
  }
  console.log(
    [
      place.en,
      best.place_name,
      best.id,
      best.y,
      best.x,
      best.category_name,
      best.road_address_name || best.address_name,
    ].join('\t'),
  )
  await sleep(100)
}
